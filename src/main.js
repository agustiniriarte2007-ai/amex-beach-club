...
function contentMap(rows){const c={...fallback};for(const r of rows){if(!r.seccion)continue;const value=r.texto||r.imagen||r.titulo||'';c[r.seccion]=value;if(/_image$/.test(r.seccion)||r.seccion==='hero_image'||r.seccion==='hero_video'){const pos=r.titulo||r.imagen_position||'center';c[`${r.seccion}_position`]=pos}}return c}
