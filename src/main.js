...
function contentMap(rows){const c={...fallback};for(const r of rows){if(!r.seccion)continue;c[r.seccion]=r.texto||r.imagen||r.titulo||'';if(r.texto&&r.titulo&&(/_image$/.test(r.seccion)||r.seccion==='hero_image'))c[`${r.seccion}_position`]=r.titulo}return c}
...
<section class="gallery" id="galeria"><div class="section-label">Galería</div><div class="gallery-grid">${gallery.map((u,i)=>`<div class="g g${i+1}" style="background-image:url('${esc(u)}');background-position:${esc(content[`gallery_${i+1}_position`]||'center')}"></div>`).join('')}</div><a href="#inicio" class="btn">Ver más</a></section>
